...
Preprocess::Preprocess()
  :feature_enabled(0), lidar_type(LIVOX), blind(1.0), point_filter_num(1)
{
  inf_bound = 10;
  N_SCANS   = 6;
  group_size = 8;
...
void Preprocess::process_cut_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out, deque<double> &time_lidar, const int required_frame_num, int scan_count)
{
    ...
    {
        if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00 || (msg->points[i].tag & 0x30) == 0x00))   //保留第0个回波
        {
            valid_point_num++;
...
void Preprocess::process_cut_frame_pcl2(const sensor_msgs::PointCloud2::ConstPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out, deque<double> &time_lidar, const int required_frame_num, int scan_count)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  if (feature_enabled)
  {
    for(uint i=1; i<plsize; i++)
    {
      if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00 || (msg->points[i].tag & 0x30) == 0x00 ))  //|| (msg->points[i].tag & 0x30) == 0x00
      {
        pl_full[i].x = msg->points[i].x;
        pl_full[i].y = msg->points[i].y;
        pl_full[i].z = msg->points[i].z;
  ...
  else
  {
    for(uint i=1; i<plsize; i++)
    {
      if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00 || (msg->points[i].tag & 0x30) == 0x00 ))  //tag主要指示回波和噪点信息，&为按位与，表示只保留第一回波
      {
        valid_num ++;
...
void Preprocess::sim_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    pl_surf.reserve(plsize);
    for (int i = 0; i < pl_orig.points.size(); i += point_filter_num) {
